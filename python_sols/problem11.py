'''
Problem 11: Given a 20 x 20 matrix, find four numbers in a row, column or
diagonally with the highest product
'''
...
def main():
    M = readInput()
    maxProd = 0
    # Just handle each row product, column product and diagonal product
    # separately
    # Column window
    for row in range(20):
        for col in range(16):
            prod = M[row][col] * M[row][col+1] * M[row][col+2] \
                    * M[row][col+3]
            if maxProd <= prod:
                maxProd = prod
    # Row window
    for row in range(16):
        for col in range(20):
            prod = M[row][col] * M[row+1][col] * M[row+2][col] \
                    * M[row+3][col]
            if maxProd <= prod:
                maxProd = prod
    # Right diagonal window
    for row in range(16):
        for col in range(16):
            prod = M[row][col] * M[row+1][col+1] * M[row+2][col+2] \
                    * M[row+3][col+3]
            if maxProd <= prod:
                maxProd = prod
